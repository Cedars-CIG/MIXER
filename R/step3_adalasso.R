...
adaptive_LASSO_selection <- function(y, geno, df_weight, lambda,
                                     family = "binomial") {
  # y: response vector
  # geno: genotype matrix/data.frame (rows: samples, cols: SNPs)
  ...
  if (length(common_snps) == 0) {
    stop("No overlapping SNPs between geno and df_weight in adaptive_LASSO_selection().")
  }
...
adaptive_LASSO_fit <- function(y, geno, df_weight,
                               lam_min, lam_max,
                               nlambda = 100,
                               family = "binomial",
                               nfolds = 20) {
  ...
  common_snps <- intersect(colnames(x), df_weight$SNP)
  if (length(common_snps) == 0) {
    stop("No overlapping SNPs between geno and df_weight in adaptive_LASSO_fit().")
  }
  x <- x[, common_snps, drop = FALSE]
  w <- df_weight$weight[match(common_snps, df_weight$SNP)]
...
run_adaptive_LASSO <- function(
  ...
  x_train <- as.matrix(geno_train)
  
  common_snps <- intersect(colnames(x_train), df_weight$SNP)
  if (length(common_snps) == 0) {
    stop("No overlapping SNPs between geno_train and df_weight in run_adaptive_LASSO().")
  }
